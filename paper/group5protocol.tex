...
\subsubsection{SessionMessage}
	...
	\item[Description] \hfill \\
		...
		authentication challenge value sent from the server to the client.
		This message signals the transition from the Connecting State to the Authenticating state.
	\item[Message Format] \hfill \\
	\begin{tabular}{ | c | c | c | c | c | c |}
		\hline
		1 byte & 1 byte & 1 byte & 1 byte & $n$-bytes &  int \\
		\hline
		Message ID & Session ID &  Version Number & Type Length & Stream Type & Challenge \\
		\hline
	\end{tabular}
	\item[Message Field Definitions] \hfill \\
	\begin{tabular}{ | p{3cm} | p{1.5cm} | p{8cm} | }
